 ...
 * Examples and docs at: https://github.com/bjorsq/inlinemultiselect
 ...
 * 												  Default value: {}
 *
 * @option String replaceForAll (optional)        Whether to replace the full list of items when they are all selected with a single word. 
 *                                                If this is set to anything other than the empty string, when all possible options have
 *                                                been selected, the list is replaced by this string.
 *                                                Default value: ''
 *
 ...
 *
 * @option Array alerts (optional)                Alert messages which are used when the limit of selections is reached
 *                                                Default value: ['Sorry, there is a limit of %d possible selection(s) from this list', 'Sorry, you cannot choose any more.']
 *
...
(function($) {
    $.fn.inlinemultiselect = function(options){
        ...
        var options = $.extend({
            ...
            alerts: ['Sorry, there is a limit of %d possible selection(s) from this list', 'Sorry, you cannot choose any more.'],
        ...
        return this.each(function(){
            ...
            if (this.type === 'select-multiple') {
    			...
    			logToConsole("Found multiple select with name: "+msname);
    			...
    			logToConsole("separator for "+msname+": "+s);
    			logToConsole("end separator for "+msname+": "+es);
				/* wrapper */
				...
				if (options.showAllLink) {
				    $('<a href="#">all</a>').addClass(options.controlsClass).click(function(){
				    	...
				    	if (options.maxSelected[groupID] && options.maxSelected[groupID] < checkboxgroup.length) {
				    		alert(options.alerts[0].replace('%d', options.maxSelected[groupID]));
				    	} else {
				...
				var formatOptionStr = function(existing, options, available){
		        	var sep = options.separatorSpecial[options.msname]? options.separatorSpecial[options.msname]: options.separator;
		        	var endSep = options.endSeparatorSpecial[options.msname]? options.endSeparatorSpecial[options.msname]: options.endSeparator;
	                var selectedStr = '';
	                if (existing.length == available && options.replaceForAll != '') {
	                	selectedStr = '<span>' + options.replaceForAll + '</span> '
	                } else if (existing.length == 1) {
		                selectedStr = '<span>' + existing[0] + '</span> ';
		            } else if (existing.length == 2) {
		                selectedStr = '<span>' +existing.join('</span>'+endSep+'<span>') + '</span> ';
		            } else {
		            	selectedStr = '<span>';
	                    for (i = 0; i < existing.length; i++) {
		                    if (i == (existing.length - 1)) {
		                        selectedStr += '</span>' + endSep + '<span>';
		                    } else if (i > 0) {
		                        selectedStr += '</span>' + sep + '<span>';
		                    }
		                    selectedStr += existing[i] + '</span>';
		                }
	                    selectedStr += " ";
		            }
    	            return selectedStr;
		        ...
		        var openStr = $('<span/>').append($('<strong id="'+msname+'choices">'+disabledStr+formatOptionStr(existingChoices,options,choicesAvailable)+' </strong>')).append(openLink);
			    /* go through options and construct checkboxes */
				$('option',this).each(function(idx){
                    var value = $(this).attr('value');
                    var text = $(this).text();
        			logToConsole("Found option ("+text+") in multiple select "+msname+" with value: "+value);
					...
					checkbox.bind("click change", function(){
				        ...
				        $('#'+msname+'choices').html(formatOptionStr(existingChoices,options,choicesAvailable));
				        /* execute callback function */
        			...
        			logToConsole("Added checkbox and label to selectList for item: "+$(this).text());
				});
				$(wrapper).hide();
				$('body').append(wrapper);
		        $(this).before(openStr);
		        logToConsole("Selected items string added to text before select element, and options popup added to form");
		        $(this).remove();
				logToConsole("Multiple select removed from DOM");
			} else {
                logToConsole("Not a multiple select: "+this);
            }
        });
    }
})(jQuery);
